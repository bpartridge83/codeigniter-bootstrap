...
class Mongo_db {
	...
	*/
	...
	public function limit($x = 99999)
	{
		...
		{
			$this->limit = (int) $x;
			array_push($this->pipeline, array('$limit'=>$x));
		}
		return ($this);
	}
	...
	*	--------------------------------------------------------------------------------
	*
	*	Runs a MongoDB aggretation See the MongoDB documentation for more usage scenarios:
	...
	*/
	...
	{
        array_push($this->pipeline, array('$match'=>$match));
	...
	{
        array_push($this->pipeline, array('$project'=>$project));
	...
	{
        array_push($this->pipeline, array('$group'=>$group));
	...
	{
        array_push($this->pipeline, array('$sort'=>$sort));
	...
	{
        if ($skip > 0) {
            array_push($this->pipeline, array('$skip'=>$skip));
        }
	...
	public function aggregate($collection = '')
	{
        if (empty($collection))
		{
			show_error("No Mongo collection selected to aggregate with", 500);
		}
        ...
        if (!isset($this->expires)) {
            return $this->aggregateCmd(array($query));
        }
        
        $results = $this->CI->cache->mongo('aggregateCmd', array($query), $this->expires);
    ...
    public function aggregateCmd($query)
    {
        $result = $this->command($query[0]);
