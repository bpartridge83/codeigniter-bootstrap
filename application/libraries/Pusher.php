...
class Pusher
{
	...
	*/
	...
	{
        //$auth_key, $secret, $app_id, $debug = false, $host = 'http://api.pusherapp.com', $port = '80', $timeout = 30
		...
		$this->settings['auth_key']	= $this->CI->config->item('auth_key');
		$this->settings['secret']	= $this->CI->config->item('secret');
		$this->settings['app_id']	= $this->CI->config->item('app_id');
		$this->settings['url']		= '/apps/' . $this->settings['app_id'];
		$this->settings['debug']	= $this->CI->config->item('debug');
